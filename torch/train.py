...
parser.add_argument('--num_iters_per_level', type=int, default=2000,
                    help='#iters before fading in training for next level.')
...
parser.add_argument('--scheduler_step_size', type=int, default=0,
                    help='#iters before scheduler step (0 for each epoch)')
...
if MotionCompletion:  # DO MOTION COMPLETION'''
    train_dataset = dataloader.MotionCompleteDataset(train_files, args.input_dim, args.truncation,
                                                     args.num_hierarchy_levels, 0)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                                                   num_workers=2, collate_fn=dataloader.collate_mocomplete)
else:  # DO 4D COMPLETION'''
    train_dataset = dataloader.MotionShapeDataset(train_files, args.input_dim, args.truncation,
                                                  args.num_hierarchy_levels, 0)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                                                   num_workers=6, collate_fn=dataloader.collate_motionshape)

# if len(val_files) > 0:
has_val = False
if has_val:
    val_dataset = dataloader.MotionShapeDataset(val_files, args.input_dim, args.truncation, args.num_hierarchy_levels,
                                                0)
    print('val_dataset', len(val_dataset))
    val_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=6,
                                                 collate_fn=dataloader.collate_motionshape)
...
def get_loss_weights(iter, num_hierarchy_levels, num_iters_per_level, factor_l1_loss, log_IO):
    ...
    if cur_level + 1 < num_hierarchy_levels:
        ...
        l1_weight = 1.0
    weights[-1] = l1_weight
    if iter % num_iters_per_level == 0 or (fade_level >= num_iters_per_level - fade_amount + step_factor and (
            fade_level - num_iters_per_level + fade_amount) % step_factor == 0):
        log_IO.write_to_terminal(f'[iter {iter}] updating loss weights: {weights}')
    return weights
...
def train_one_epoch_motion_shape_complete(epoch, iter, dataloader, log_IO):
    ...
    for t, sample in enumerate(dataloader):
        ...
        shape_loss_weights = get_loss_weights(iter, args.num_hierarchy_levels, args.num_iters_per_level,
                                              args.weight_sdf_loss, log_IO)
        ...
        if pred_shape is not None:
            ...
            shape_loss, shape_loss_hier = loss_util.compute_shape_loss(
                ...
                shape_loss_weights,
                args.truncation, args.logweight_target_sdf, args.weight_missing_geo, inputs[0], args.use_loss_masking,
                known)
        ...
        # Todo: some error of varience of output of pred_motion shape, need to fix for using motion
        if pred_motion is not None and False:
