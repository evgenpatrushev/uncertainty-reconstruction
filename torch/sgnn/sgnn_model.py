...
class SparseEncoderLayer(nn.Module):
    def __init__(self, nf_in, nf, input_sparsetensor, return_sparsetensor, max_data_size):
        ...
        self.p2.add(scn.ConcatTable()  #residual block?
                    .add(scn.Identity())
                    .add(scn.Sequential()
    ...
    def forward(self,x):
        if not self.input_sparsetensor:
            x = self.p0(x)  # convert [corrd, value] to SCN tensor
...
class TSDFEncoder(nn.Module):
    ...
    def forward(self,x):
        ...
        return x, out, feats_sparse,  bottleneck, dec0
...
class DeepSDFHead(nn.Module):
    ...
    def forward(self, x):
        ...
        loc_feats = locs[:, :3]
        loc_feats = loc_feats.type(torch.FloatTensor).cuda()
        loc_feats = torch.cat([loc_feats % 32,
                               loc_feats % 16,
                               loc_feats % 8,
                               loc_feats % 4,
                               loc_feats % 2], 1)

        x =  torch.cat ( [x, loc_feats], 1).T
...
class GenModel(nn.Module):
    def __init__(self, encoder_dim, input_dim, input_nf, nf_coarse, nf, num_hierarchy_levels, pass_occ, pass_feats, use_skip_sparse, use_skip_dense, truncation=3):
        ...
        if self.PRED_SURF:
            ...
            if do_implicite :
                pass
                self.surfacepred = DeepSDFHead(nf_in, nf, nf_out, self.refine_sizes[-1])
        ...
        # self.implicit_sdf = True
        # if self.pred_implict_surface:
        #     pass
    ...
    def update_sizes(self, input_max_dim, refine_max_dim):
        ...
        for k in range(3):
            ...
            for h in range(len(self.refinement)):
                self.refinement[h].p0.spatial_size[k] = torch.tensor(refine_max_dim[k])
                refine_max_dim *= 2
                self.refinement[h].n0.spatial_size[k] = torch.tensor(refine_max_dim[k])
    ...
    def forward(self, x, loss_weights):
        ...
        # encode
        x, out, feats_sparse, bottleneck, dec0 = self.encoder(x)
        batch_size = x.shape[0]
        if self.use_skip_sparse:
            for k in range(len(feats_sparse)):
                '''convert SCN tensor to [location, value] tuple''' #print('[model] feats_sparse[%d]' % k, feats_sparse[k].spatial_size)
        ...
        # loss_weights = [1,1,1,1,1] # for debug
