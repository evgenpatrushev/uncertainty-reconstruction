...
class MotionShapeCompleteModel(nn.Module):
    def __init__(self, input_dim, pass_occ, pass_feats, use_skip_sparse,
        ...
        self.refine_sizes = [(np.array(input_dim) // (pow(2, k))).tolist() for k in range(num_hierarchy_levels - 1)][
                            ::-1]
        ...
        for h in range(1, num_hierarchy_levels):
            ...
            self.shape_decoder.add(ShapeDecoder(nf_in, nf_shape_decoder, pass_occ, pass_feats, self.refine_sizes[h - 1],
        ...
        log_IO.write_to_terminal('#params shape decoder: ' +
                                 str(count_num_model_params(self.shape_decoder) + count_num_model_params(
                                     self.surfacepred)))
    ...
    def dense_coarse_to_sparse(self, coarse_feats, coarse_occ, truncation):
        ...
        # sparse locations
        locs_unfilt = torch.nonzero(
            torch.ones([coarse_occ.shape[2], coarse_occ.shape[3], coarse_occ.shape[4]])).unsqueeze(0).repeat(
            coarse_occ.shape[0], 1, 1).view(-1, 3)
        batches = torch.arange(coarse_occ.shape[0]).to(locs_unfilt.device).unsqueeze(1).repeat(1, coarse_occ.shape[2] *
                                                                                               coarse_occ.shape[3] *
                                                                                               coarse_occ.shape[
                                                                                                   4]).view(-1, 1)
